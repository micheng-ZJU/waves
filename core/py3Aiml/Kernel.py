...
class Kernel:
    ...
    def loadSubs(self, filename):
        """"加载替换文件。该文件必须采用Windows风格的INI格式（有关此格式的信息，请参阅标准的ConfigParser模块文档）。
        文件的每个部分都被加载到自己的替代者中。        """
        parser = ConfigParser()
    ...
    def respond(self, input_, sessionID=_globalSessionID):
        ...
        try:
            ...
            finalResponse = finalResponse.lstrip().rstrip(" ")
    ...
    def _respond(self, input_, sessionID):
        ...
        if elem is None:
            ...
            # 将元素处理为响应字符串。
            response += self._processElement(elem, sessionID).lstrip().rstrip(" ")
            response += u" "
